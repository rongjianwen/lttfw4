        ...
        ElementNode.replace(v, {
            sidebar: {
                element: (props: any) => {
                    ...
                    const outlinerWidth = useSelector((state: any) => state.outliner.width);

                    const styledElement = StyledComponent('div', (theme: any) => ({
                        width: theme.sidebar.width,
                        backgroundColor: theme.sidebar.backgroundColor,
                        color: theme.sidebar.color,
                        [theme.breakpoints.down('xs')]: {
                            width: `calc(100% - ${outlinerWidth}px)`,
                            height: `calc(100% - ${theme.header.height})`,
                            position: 'absolute',
                            backgroundColor: theme.header.backgroundColor
                        }
                    }));
                    const newProps = { ...props, styledElement };
                    return open && <Panel {...newProps} />;
                }
            }
        });
    }
